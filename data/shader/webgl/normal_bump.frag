...
void main() {
    if (waterlineClip > 0.5  && FragPos.y <  waterlineY) discard;
    if (waterlineClip < -0.5 && FragPos.y >= waterlineY) discard;
    ...
    vec3 bumpNormal = normalize(normal + vec3(0.0, 0.0, height * bumpFactor));

    vec3 finalNormal = normalize(bumpNormal);
